...
![Abb. 1.a.4: Jobs als Programmierer im Ranking](topics/Introduction/entwickler-report-programmierer-stack-overflow-2-620x483.jpg.webp)
...
Für Kinder und Jugendliche gibt es z.B. mit *Scratch* und *Game Builder Garage* für die
*Nintendo Switch* Umgebungen zum Erlernen von Basisfähigkeiten der (Spiele-) Programmierung.

![Abb. 1.a.6: Säulen der Low Code Entwicklung](topics/Introduction/Low_Code_Ziele.png)

##### Projektmanagement

Laut der **DIN 69901-5:2009** ist ein Projekt ein Vorhaben, das im Wesentlichen durch Einmaligkeit 
der Bedingungen in ihrer Gesamtheit gekennzeichnet ist (im Gegensatz zur Linienproduktion).
...
Die nun folgenden **5 Formen der Datenanalyse** unterscheiden sich in ihren Zielen,
Ihren Methoden und den daraus gewonnenen Erkenntnissen.

##### a) Die deskriptive Datenanalyse

Die deskriptive Datenanalyse, auch als **beschreibende Datenauswertung** bezeichnet, konzentriert sich auf die **Daten aus der Vergangenheit**. Sie ordnet
...
die **Informationen aggregieren**, **ordnen** und **strukturieren**. Die deskriptive
...
Ziel der explorativen Datenanalyse ist es, **Zusammenhänge in Daten** zu finden
...
**Digitalisierung** bezeichnetdas Umwandeln von analogen Werten in ein digitales Format bzw. digitale
Repräsentation), welche zu verarbeiten sind.

Hierzu werden **Daten** IT-technisch gespeichert, intechnisch gespeichert, in
Form von Bits & Bytes. Diese werden in verschiedenen Stellenwertesystemen und
...
Die kleinste Einheit ist *1 Bit*. “Bit” steht für “Binary Digit”. In der Informatik wird im
Binärsystem gerechnet, was bedeutet, dass es nur zwei Zustände gibt – *an /
aus* bzw. *0 / 1*. Der Computer rechnet mit diesen Zahlen, dem binären
...
**ISO-8859:** Set von Zeichensätzen, die auf *8* Bit basieren und *256* Zeichen abbilden. Die ersten *128* Zeichen (0 - 127) sind immer identisch mit
dem ASCII-Zeichensatz. Die zweiten *128* Stellen enthalten spezielle Zeichen.
Z.B. ISO-8859-1: schriftspezifische Zeichen für westeurop. / amer. Sprachen

**Unicode:** Universeller Zeichensatz zur Darstellung aller Zeichen aller Sprachen.
*16* Bit gleich *65536* Zeichen (erste Byte auf null gesetzt -\> ASCII / ANSI)
...
**CRLF:** Unter Windows-Systemen wird mit *CR* eingeleitet (auch mit *Carriage
...
Weiterhin muss je nach Anwendungsfall der eigentliche **Ort der Datenanalyse** ausgewählt werden. Je nach Anforderung ist es sinnvoll Daten zentral zu
...
von Geschäftsprozessen heranziehen.
...
1. **Business Intelligence** „Was ist in der Vergangenheit
...
vorherzusagen.
2. **Business Analytics** „Warum ist etwas geschehen und
...
**systematische Erstellen** von **Berichten** auf Basis relevanter Unternehmensdaten\
und Informationen.
...
auch wegen der **Vielfalt** und **Komplexität** ihrer **Beschaffenheit**.

![Abb. 1.d.3: Aktivität eines Wikipedia-Bots](topics/Introduction/Viegas-UserActivityonWikipedia.gif)
...
![Abb. 2.a.1: Buch – Einführung in die Datenverarbeitung](topics/Introduction/buch_datenverarbeitung.jpeg)
...
rechtwinkligen Achsen der Zeilen und Spalten hält im Kopf die **Variablennamen** und in den einzelnen Zeilen, die jeweils dem Merkmal zugordneten **Objekte**.
...
Hinblick auf **Performance** konzipiert werden,\
müssen diese Daten\
...
-   eigene R-Pakete (mit eigenen Daten oder Funktionen lassen sich mittels
...
\# Installations-Skript ausführen (ca. 20 Min., App muss im **Fokus** bleiben und der Bildschirm darf nicht ausgehen, sonst bricht die\
...
2.  
    ...
    2.  Datensatz suchen (z.B. auf Webseiten wie Statista ([https://de.statista.com)\
...
Will man einen **Zusammenhang** zwischen **zwei metrischen Variablen** untersuchen, zum Beispiel zwischen dem Alter und dem Gewicht von\
...
\# Cluster Denrogramm* res.hc \<- hclust(dist(mydata), method = "ward.D2")\
fviz_dend
...
Die informationsverarbeitenden Einheiten arbeiten nicht auf eine *lineare* Weise. Vielmehr bezieht das neuronale Netz seine Stärke aus\
...
-   *Entweder* **eigene Daten erheben** (evtl. die Excel aus dem Unterricht
...
worden sein!) über **12 - 15 Seiten** -\> per Mail an mich und auf Ilias\
...
-   Ergebnisse als **Präsentation** bzw. als **„Pitch“** (ca.15 – 20 Min.)
...
Romeijn, J. W. \[2016\]\
Philosophy of Statistics. In E. N. Zalta (Hrsg.), The Stanford Encyclopedia of\
...
Tukey, J. W. \[1962\]\
*The future of data analysis. The Annals of Mathematical Statistics, Vol. 33,\
