...
![Vorstellung](topics/Introduction/Vorstellung.png)
...
Excel-Makro-Datei (xlsm) wird regelmäßig aktualisiert, da sie sich in der\
...
![Gliederung](topics/Introduction/Gliederung.png)
...
**Definition Statistik:** Wissenschaft von Sammlung, Analyse und Interpretation und Kommunikation\
von Daten mithilfe mathematischer Verfahren zur Entscheidungsfindung.1

**Definition Datenanalyse:** Der Prozess des Erhebens von Daten, ihrer Auswertung und Interpretation.2

**Definition Datenverarbeitung:** Der Prozess des Datensammelns und deren Umwandlung in nützliche\
...
![Abb. 1.a.1: Microsoft Windows 1.01](topics/Introduction/windows1.0.jpg)
...
![Abb. 1.a.2: IBM System/360](topics/Introduction/ibm-360.jpg)
...
![Abb. 1.a.2: Betriebssystem Unix Timeline](topics/Introduction/unix_timeline.png)

##### Programmiersprachen und Erscheinungsjahr

![Abb. 1.a.3: Programmiersprachen im Ranking](topics/Introduction/Programmiersprachen.jpg)
...
![Abb. 1.a.4: Jobs als Programmierer im Ranking](topics/Introduction/entwickler-report-programmierer-stack-overflow.png)
...
![Abb. 1.a.5: Microsoft Visual Studio](topics/Introduction/VSCode.png)
...
![Abb. 1.a.6: Säulen der Low Code Entwicklung](topics/Introduction/Low_Code_Ziele.png)
...
![Abb. 1.a.7: Projektmanagement Prozess](topics/Introduction/projektmanagement.png)
...
![Abb. 1.a.8: SCRUM Manifesto](topics/Introduction/scrum.png)
...
![Abb. 1.a.6: Rolle der Daten der Digitalisierung](topics/Introduction/Wertbeitrag_der_Daten.png)
...
![Abb. 1.b.2: ASCII Zeichensatz](topics/Introduction/ASCII.png)
...
![Abb. 1.b.3: Tabelle der Speichergrößen](topics/Introduction/bit_byte.png)
...
![Abb. 1.b.4: Stellenwertsysteme](topics/Introduction/hexadezimal.png)
...
**XML:** e**X**tensible **M**arkup **L**anguage, wie bei HTML sind Daten von Markierungen umgeben. Es sind Struktur und Bedeutungen zu vergeben. Bsp.:
...
![Abb. 1.b.5: NotePad++ - CRLF](topics/Introduction/CRLF.png)
...
![Abb. 1.b.7: Methoden zur Extraktion von Informationen](topics/Introduction/Datenverarbeitung.png)
...
![Abb. 1.b.8: Der Rahmen der Datenanalyse](topics/Introduction/5_Schritte_Datenanalyse.png)
...
![Abb. 1.c.1: Der Rahmen der Datenanalyse](topics/Introduction/customer-service-team-dashboard.png)
...
![Abb. 1.d.1: Formen der Datenanalyse](topics/Introduction/bi-analytics2.png)
...
![Abb. 1.d.2: Dashboard einer Reporting Software](topics/Introduction/analytics-dashboard-reporting-3.png)
...
![Abb. 1.d.3: Aktivität eines Wikipedia-Bots](topics/Introduction/Viegas-UserActivityonWikipedia.png)
...
![Abb. 2.a.1: Buch – Einführung in die Datenverarbeitung](topics/Technical%20Applications/buch_datenverarbeitung.jpg)
...
![Abb. 2.a.2: Zeitschiene *Microsoft Excel*](topics/Technical%20Applications/Excel.png)

#### Grundbegriffe

**Daten** (die Einzahl Datum ist ungewöhnlich) kann man als Informationen, die eine Funktion haben, beschreiben.1 Somit sind, von für den Menschen nützliche Daten, beliebige Daten ohne Zweck und Systematik ausgenommen. Der Zweck muss demnach zumindest Fachpersonal bekannt und gängigen Methoden der **Datenerfassung** und **Datenverarbeitung** zugänglich sein.
...
Objekt 1 M1 | Objekt 1 M2 | Objekt 1 M3
Objekt 2 M1 | Objekt 2 M2 | Objekt 2 M3
Objekt 3 M1 | Objekt 3 M2 | Objekt 3 M3
...
![Abb. 2.b.1: Primär- und Sekundärdaten](topics/Technical%20Applications/primary_secondary_data.png)
...
Ein **Data Warehouse** ist eine Art **Datenmanagementsystem**, mit dem BI-
...
**wertvolle Geschäftsinformationen** aus ihren Daten abzuleiten, um die
...
Es können Datenbestände aus verschiedenen Teilmengen zusammengestellt und bedarfsgerecht für Anwendungsprogramme und
deren Benutzern angezeigt werden.

Heutzutage kann eine Datenbank viele Informationen beinhalten. Komplexe Datenbanken werden oft
in Warenwirtschaftssystemen (abgekürzt WWS oder WaWi) verwendet oder auch zur Verarbeitung von
Sozial- und Gesundheitsdaten (Sozialversicherungsträger, Sozialhilfeträger,
Versorgungsbehörden, Haus- und Fachärzte).

### 2.c) Speicherung der Daten, Datenbanken, Datawarehouses

![Abb. 2.c.1: Datenbanksysteme](topics/Technical%20Applications/datenbank.jpg)
...
Interaktionen und Verhalten (z.B Highscores).

Da Spiele oftmals im Hinblick auf **Performance** konzipiert werden,
müssen diese Daten effizient und konsistent gespeichert werden.
Hierzu eigenen sich **Datenbanksysteme** um große Datenmengen zu
speichern.
...
**Fragen an Lukas der als Gast-Dozent zu einer Vorlesung kam** (er ist
...
### 2.e) Einführung in verschiedene Anwendungen und deren Darstellung mit Vor- und Nachteilen
...
**Vereinfachen** Sie den Zugriff auf traditionelle und neue Daten. Mehr Daten
bedeuten im Allgemeinen bessere Prädiktoren.

**Stärken des Data Scientists** mit fortschrittlichen Analysetechniken. Die
...
**Bereinigen der Daten**, um Qualität in bestehende Prozesse zu integrieren.
Bis zu 40 Prozent aller strategischen Prozesse scheitern an schlechten
Daten.

**Gestalten der Daten** mit flexiblen Manipulationstechniken. Die Vorbereitung
...
möglichst autonome und effiziente Identifizierung und Beschreibung von **interessanten
Datenmustern** aus großen Datenbeständen ermöglicht.*
...
können **Data Mining Prozesse** und **Algorithmen** einfach konfiguriert
und die Ergebnisse zur leichten Verständlichkeit visualisiert werden.

Ein Beispiel ist ***Knime.*** Die
Datenmanipulation wird visuell
anhand einzelner **Funktionsknoten** dargestellt und kann individuell
beschriftet werden. Dadurch werden die einzelnen Prozessschritte greifbar und der komplette
...
Datenanalyse wird am häufigsten die Programmiersprache ***Python*** verwendet.
...
Zur Mustererkennung in Daten kommen **Algorithmen** aus der **Statistik**, **Mathematik** und
...
Häufig ist man an einer Klassifizierung eines Dokuments, beispielsweise nach Themengebiet oder fachlichem Niveau, interessiert. Ein Ansatz ist, zunächst relevante
...
müssen herausgefiltert und Abkürzungen erkannt werden.
...
und Interessen der Online-Nutzer zu erhalten. \[…\]
...
**4. Erarbeiten IT-technischer Rahmenbedingungen
4.a) Anleitung / Installation der Open Source Lösung auf den Rechner der
...
Windows** Für das Microsoft Windows Betriebssystem hat sich der kostenlose Editor ***Notepad++*** in
Funktionsumfang, Performance und Anwenderfreundlichkeit einen deutlichen Vorsprung gegenüber
der Konkurrenz erarbeitet.
*Notepadd++* herunterladen unter:<https://notepad-plus-plus.org/downloads/>

##### MacOS

Für das Apple MacOS Betriebssystem kann man sehr gut auf den kostenlosen Editor ***Brackets*** von
Adobe zurückgreifen. *Brackets* herunterladen unter: [https://brackets.de.uptodown.com/mac
](https://brackets.de.uptodown.com/mac)Unter[https://registry.brackets.io](https://registry.brackets.io/) lassen sich eine Vielzahl an Erweiterungen downloaden und unter *Datei – Erweiterungsverwaltung* installieren. Für die Vorlesung sind wichtig:
...
***R*** herunterladen unter[: https://cran.r-project.org/bin/macosx/](https://cran.r-project.org/bin/macosx/)(MacOS) oder
...
und *R-Paketen iSH-AOK* starten und folgende Befehle in die Eingabeaufforderung tippen:
...
\# Installations-Skript ausführen (ca. 20 Min., App muss im **Fokus** bleiben und der Bildschirm darf nicht ausgehen, sonst bricht die Installation evtl. ab! Tipp – regelmäßig etwas **Scrollen**)
...
*Dateien* App in den **Splitscreen** an den Bildschirmrand hinzu-
gefügt werden.

In der Dateien App muss vorher
das **Laufwerk** *ish* **aktiviert** werden. Die Ausgabe-Dateien
liegen unter ish
/home/ish/R/

Nach der Ausgabe eines
Diagrammes zum **Aktualisieren** des Ordnerinhaltes einmal\
...
\# Daten aus XML Datei einlesen d2 \<- xmlToDataFrame("Fragebogen_Kopfumfang_Fach.xml") \# XML Datei als Data Frame einlesen print(d2) \# d2 ausgeben summary(d2\$PRAXIS) \# Min. / Max., Median und arithmetisches Mittel berechnen \# Diagramme erstellen, dass die (mögliche) Abhängigkeit zweier numerischer Variablen zeigt plot(d2\$NOTE_MATHE \~ d2\$KOPF, ylab = "Mathe Note", xlab = "Kopfumfang", ylim = range(1:6)) boxplot(d2\$NOTE_MATHE \~ d2\$KOPF, ylab = "Mathe Note", xlab = "Kopfumfang", ylim = range(1:6))

\# Skriptdatei (aus R Studio) in R Konsole ausführen source(“Fragebogen.R“)

## 4. Erarbeiten IT-technischer Rahmenbedingungen

### 4.b) Einführung und Anleitung der Applikation *R*

**Pakete in *R* installieren** (Datei *Pakete_in_R_installieren.R* auf Ilias) \# XML Paket installieren install.packages("XML")
...
install_github(“DrBenjamin/ourdata") library(ourdata) ourdata() help(ourdata) print(fragebogen)
...
Für *R* auf dem *iPad* sind die Pakete mittels Installationsskript (*install4R.sh*)
...
\# Einfacher Vektor, die Funktion c kombiniert Werte in einen Vektor oder eine Liste \# fruitVec wird mit 4 Werten (Strings) gefüllt fruitVec \<- c("Apfel", "Banane", "Orange", "Birne")

\# Einfache While-Schleife \# Funktion length: gibt die Länge eines/r Vektors / Liste aus \# Der Vektor fruitVec kann wie ein Array angesprochen werden, um die einzelnen Werte auszulesen
pos \<- 1 while (pos \<= length(fruitVec)) {

print(paste0("Die Frucht Nr. ", pos, " ist : ", fruitVec\[pos\])) pos \<- pos + 1
}
...
Analyse
R-Analyse-Workshop** Verwende beliebige Daten (aus dem Internet), um eine weitere **Korrelations-
...
2.  Datensatz suchen (z.B. auf Webseiten wie Statista ([https://de.statista.com)](https://de.statista.com/) aufbereiten (Dezimaltrennung, Variablennamen, Spalten bereinigen)
...
**Aufgabe:** Programmiere die VBA-Funktion Zusammenführen aus der Excel
...
Will man einen **Zusammenhang** zwischen **zwei metrischen Variablen** untersuchen, zum Beispiel zwischen dem Alter und dem Gewicht von Kindern, so berechnet man eine **Korrelation**. Diese besteht aus
einem *Korrelationskoeffizienten* (*rho* bei Spearman) und einem *p-Wert*.

Der **Korrelationskoeffizient** gibt die Stärke und die Richtung des
Zusammenhangs an. Er liegt zwischen *-1* und *1*. Ein Wert nahe *-1* bezeichnet einen **starken negativen Zusammenhang** (z.B. „*Mehr
...
Analysen können aber **Hinweise** in diese Richtung geben.
...
führen, indem sie Einzelhändlern helfen, **selektives Marketing** zu betreiben\
...
Der **K-Means-Clusteralgorithmus** ist definiert als eine *unüberwachte* **Lernmethode** mit einem iterativen Prozess, bei dem die Datensätze in eine\
...
\# Bibliotheken laden library(cluster) library(factoextra) library(pheatmap)

\# Datensatz von Verbrechern in den USA erstellen mydata \<- scale(USArrests)
...
\# Visualisieren fviz_cluster(km.res, data = mydata, palette = "jco", ggtheme = theme_minimal(), barfill = "red", barcolor = "red", linecolor = "red")

*\# Hierarchisches Clustering \# Cluster Denrogramm* res.hc \<- hclust(dist(mydata), method = "ward.D2") fviz_dend

(res.hc, cex = 0.5, k = 4, palette = "jco")

*\# Heatmap erstellen* pheatmap
...
**Neuronale Netze-Analyse in R** (Datei *Neuronale_Netze.R* auf Ilias) \# Bibliothek 'neuralnet' laden library(neuralnet)
...
max \<- apply(data , 2 , max) min \<- apply(data, 2 , min) scaled \<- as.data.frame(scale(data, center = min, scale = max - min))

### 9.b) Anwendung der Neuronale Netze-Analyse mit Beispieldaten

## 9. Methodik: Neuronale Netze-Analyse

**Neuronale Netze-Analyse in R** (Datei *Neuronale_Netze.R* auf Ilias) \## Neuronales Netz anpassen - Erstelle Trainings- und Testdaten-Set trainNN \<- scaled\[index , \] testNN \<- scaled\[-index , \]

\# Neuronales Netz anpassen set.seed(2) NN \<- neuralnet(rating \~ calories + protein + fat + sodium + fiber, trainNN, hidden = 3 , linear.output = T)
...
\# Diagramm und Regressionsgrade erstellen plot(datatest\$rating, predict_testNN, col='blue', pch=16, ylab = "predicted rating NN", xlab = "real rating") abline(0,1)

### 9.b) Anwendung der Neuronale Netze-Analyse mit Beispieldaten

Alle **Methodiken** sind mit Code und visueller Ausgabe verfügbar auf:
